...
**DEM（Digital Elevation Model）数字高程模型**： DEM 用于描述地表的地形高度信息，但不包括地表上的人工或自然物体（例如建筑物、树木等）。DEM 只考虑地表的形状和高度，通常用于地形分析、水文模拟、地貌研究等领域。

**DSM（Digital Surface Model）数字地表模型**： DSM 描述了地表上的所有表面物体（例如建筑物、树木、山地等）以及地表本身的高度信息，包括地表上的所有物体的高程。在 DSM 中，地表的高程被测量到最高点，不考虑地表下的地形。

**DTM（Digital Terrain Model）数字地形模型**： DTM 是一个更精确的地形模型，它描述了地表的形状、高程以及地表以下的地形。DTM 排除了地表上的人工和自然物体，只关注地形的实际形状。DTM 通常用于土地规划、工程设计、流域分析等需要准确地形信息的应用中。
